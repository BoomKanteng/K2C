...
<?php include('sidebar.php'); ?>
  ...
  <div class="content-wrapper">
            ...
            <div class="small-box bg-gradient-primary"> <!-- สีเดิม : bg-info -->
              <div class="inner">
                <h3><?php //ด้านบน Kcal
					...
					while($objResultAllKcal = mysqli_fetch_array($objQueryAllKcal)) {
					  $sum=$sum+$objResultAllKcal['eKcal'];
					}
					echo $sum;
					?></h3>
          ...
          <div class="col-lg-3 col-6">
            ...
            <div class="small-box bg-danger"> <!-- class="small-box bg-success" สีเขียว-->
              <div class="inner">
                ...
                while($objResultAllEnergy = mysqli_fetch_array($objQueryAllEnergy)){
                  $burn=$burn+$objResultAllEnergy['wKcal'];
              }
              echo $burn;
                ...
                <div class="tab-content p-0">
                  ...
                  <div class="chart tab-pane active" id="revenue-chart"
                       ...
                       <?php
						...
						$Alleat = "SELECT * FROM eat WHERE uID ='".$_SESSION['User']."' AND eDate='".$eDate."'";
    ...
    <p style="text-align:left"> <img src="dist/img/calendar 3.png" style=" padding: 5px; width: 50px; ">&nbsp; <?php echo $eDate;?></p>
...
$AllWorkoutShow ="SELECT * FROM workout WHERE uID='".$_SESSION['User']."' AND wDate='".$eDate."'";
...
?>
 ...
 <?php   } ?>
          ...
          <section class="col-lg-5 connectedSortable">
            ...
            <!-- การรับทานอาหาร -->
            <form name="foodform" method="post" action="">
            <div class="card bg-gradient-primary">
              <div class="card-header border-0">
                <h3 class="card-title">
                  <i class="fas ion-spoon"></i>
                  &nbsp;การรับประทาน
                </h3>
                ...
                <div class="card-tools">
                  <ul class="nav nav-pills ml-auto">
                    <li class="nav-item">
                    	<input type="submit" class="btn btn-light" name="บันทึก" value="บันทึก">
                    </li>
                  </ul>
                </div>
              </div>
              <div class="card-body">
                  ...
                  <select name="food" class="form-control select2" style="width: 100%;">
                  	<option value="">เลือกรายการอาหาร</option>
						<?php     

        
                              $sqlfoodSavory = "SELECT * FROM food ORDER BY fType,fID ASC";
                              $objQueryfoodSavory = mysqli_query($conn,$sqlfoodSavory);
                              $objResultfoodSavory = mysqli_fetch_array($objQueryfoodSavory);
							                $ftype = '';
                              while($objResultfoodSavory = mysqli_fetch_array($objQueryfoodSavory)) {
							  	              if($ftype!=$objResultfoodSavory['fType']) {
									                echo "<option disabled='disabled'>".$objResultfoodSavory['fType']."</option>";
									                $ftype = $objResultfoodSavory['fType'];
	                              }
                        ?>
            			<option value="<?php echo $objResultfoodSavory['fID'];?>"><?=$objResultfoodSavory['fName']?>  //  <?=$objResultfoodSavory['fKcal']?> kcal</option>
  						<?php } ?>   
                  </select>
                  <hr>
                </div>
            ...
            <!-- etcFood -->
                <label>หรือ กำหนดเอง</label>
                
               
                <input type="text" name="etcName" class="form-control" placeholder="ชื่ออาหารอื่นๆ"><BR>
                <input type="number" name="etcKcal" class="form-control" placeholder="แคลอรี่ (Kcal)"> 
            ...
            <div class="card bg-danger">
              ...
              <div class="card-body">
                  <div class="form-group">
                  <label>รายการใช้พลังงาน</label>
                  <select name="workout" class="form-control select2" style="width: 100%;">
                  	<option value="">เลือกรายการออกกำลังกาย</option>
						<?php  
                              $sqlworkout = "SELECT * FROM mmove ORDER BY mType,mID DESC";
                              $objQueryworkout = mysqli_query($conn,$sqlworkout);
							  $mtype = '';
                              while($objResultworkout = mysqli_fetch_array($objQueryworkout)) {
							  	if($mtype!=$objResultworkout['mType']) {
									echo "<option disabled='disabled'>".$objResultworkout['mType']."</option>";
									$mtype = $objResultworkout['mType'];
								} 
              ?>
              
            			<option value="<?php echo $objResultworkout['mID'];?>"><?=$objResultworkout['mName']?>  //  <?=$objResultworkout['mMET']?> MET</option>
  						<?php } ?> 
                  </select>

                  <BR>

              <input type="number" name="WorkoutwMinute" value="WorkoutwMinute" class="form-control" placeholder="เวลาที่ใช้ทำกิจกรรม(นาที)"> 
                 
              <hr>
                  <label>หรือ กำหนดเอง</label>

                  <input type="text" name="etcWorkoutName" class="form-control" placeholder="ชื่อกิจกรรมอื่นๆ"><BR>
                <input type="number" name="etcWorkoutKcal" class="form-control" placeholder="แคลอรี่ (Kcal)">         
